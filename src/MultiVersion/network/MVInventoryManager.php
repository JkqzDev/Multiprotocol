 ...
 */
class MVInventoryManager extends InventoryManager {
    //TODO: HACK!
    ...
    public function onClientOpenMainInventory(): void {
        ...
        if (!isset($this->openHardcodedWindows[$id])) {
            ...
            $this->session->sendDataPacket(ContainerOpenPacket::entityInv(
                self::HARDCODED_INVENTORY_WINDOW_ID,
    ...
    public function syncSlot(Inventory $inventory, int $slot, ?ItemStack $itemStack = null): void {
        $windowId = $this->getWindowId($inventory);
        if ($windowId !== null) {
