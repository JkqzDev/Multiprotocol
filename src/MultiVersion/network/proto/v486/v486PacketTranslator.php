...
class v486PacketTranslator extends PacketTranslator {
    ...
    public function handleIncoming(ServerboundPacket $pk): ?ServerboundPacket {
        if ($pk instanceof RequestChunkRadiusPacket)
            v486RequestChunkRadiusPacket::fromLatest($pk);
        return $pk;
    }
    ...
    public function handleOutgoing(ClientboundPacket $pk): ?ClientboundPacket {
        if ($pk instanceof AddActorPacket)
            return v486AddActorPacket::fromLatest($pk);
        ...
        if ($pk instanceof AddPlayerPacket)
            return v486AddPlayerPacket::fromLatest($pk, $this->blockMapping, $this->itemTranslator);
        if ($pk instanceof AddVolumeEntityPacket)
            return v486AddVolumeEntityPacket::fromLatest($pk);
        if ($pk instanceof AvailableActorIdentifiersPacket)
            return $this->availableActorIdentifiers;
        if ($pk instanceof AvailableCommandsPacket)
            return v486AvailableCommandsPacket::fromLatest($pk);
        if ($pk instanceof BiomeDefinitionListPacket)
            return $this->biomeDefs;
        if ($pk instanceof ClientboundMapItemDataPacket)
            return v486ClientboundMapItemDataPacket::fromLatest($pk);
        if ($pk instanceof CraftingDataPacket)
            return $this->craftingDP;
        ...
        if ($pk instanceof NetworkChunkPublisherUpdatePacket)
            return v486NetworkChunkPublisherUpdatePacket::fromLatest($pk);
        if ($pk instanceof NetworkSettingsPacket)
            return v486NetworkSettingsPacket::fromLatest($pk);
        if ($pk instanceof RemoveVolumeEntityPacket)
            return v486RemoveVolumeEntityPacket::fromLatest($pk);
        if ($pk instanceof SetActorDataPacket)
            return v486SetActorDataPacket::fromLatest($pk);
        if ($pk instanceof SpawnParticleEffectPacket)
            return v486SpawnParticleEffectPacket::fromLatest($pk);
        if ($pk instanceof StartGamePacket)
            return v486StartGamePacket::fromLatest($pk);
        ...
        if ($pk instanceof UpdateAdventureSettingsPacket)
            return null;
