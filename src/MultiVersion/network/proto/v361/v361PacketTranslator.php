...
class v361PacketTranslator extends PacketTranslator {
    ...
    public function handleIncoming(ServerboundPacket $pk): ?ServerboundPacket {
        ...
        if ($pk instanceof v361ContainerClosePacket) {
            if ($pk->windowId === 255)
                $pk->windowId = WindowTypes::WORKBENCH;
            return $pk;
        }
        if ($pk instanceof v361PlayerActionPacket) {
            if ($pk->action === 13)
                $pk->action = PlayerAction::RESPAWN;
            return $pk;
        }
    ...
    public function handleOutgoing(ClientboundPacket $pk): ?ClientboundPacket {
        if ($pk instanceof AddActorPacket)
            return v361AddActorPacket::fromLatest($pk);
        ...
        if ($pk instanceof AddPlayerPacket)
            return v361AddPlayerPacket::fromLatest($pk, $this->blockMapping);
        if ($pk instanceof AvailableActorIdentifiersPacket)
            return new v361AvailableActorIdentifiersPacket();
        if ($pk instanceof AvailableCommandsPacket)
            return v361AvailableCommandsPacket::fromLatest($pk);
        if ($pk instanceof BiomeDefinitionListPacket)
            return new v361BiomeDefinitionListPacket();
        if ($pk instanceof ContainerClosePacket)
            return v361ContainerClosePacket::fromLatest($pk);
        if ($pk instanceof CraftingDataPacket)
            return clone $this->craftingDP;
        if ($pk instanceof CreativeContentPacket)
            return $this->creativeContent;
        if ($pk instanceof HurtArmorPacket)
            return v361HurtArmorPacket::fromLatest($pk);
        ...
        if ($pk instanceof MovePlayerPacket)
            return v361MovePlayerPacket::fromLatest($pk);
        if ($pk instanceof NetworkChunkPublisherUpdatePacket)
            return v361NetworkChunkPublisherUpdatePacket::fromLatest($pk);
        if ($pk instanceof PhotoTransferPacket)
            return v361PhotoTransferPacket::fromLatest($pk);
        if ($pk instanceof PlayerListPacket)
            return v361PlayerListPacket::fromLatest($pk);
        if ($pk instanceof PlayerSkinPacket)
            return v361PlayerSkinPacket::fromLatest($pk);
        if ($pk instanceof ResourcePackChunkDataPacket)
            return v361ResourcePackChunkDataPacket::fromLatest($pk);
        if ($pk instanceof ResourcePacksInfoPacket)
            return v361ResourcePacksInfoPacket::fromLatest($pk);
        if ($pk instanceof ResourcePackStackPacket)
            return v361ResourcePackStackPacket::fromLatest($pk);
        if ($pk instanceof RespawnPacket)
            return v361RespawnPacket::fromLatest($pk);
        if ($pk instanceof SetActorDataPacket)
            return v361SetActorDataPacket::fromLatest($pk);
        if ($pk instanceof SetSpawnPositionPacket)
            return v361SetSpawnPositionPacket::fromLatest($pk);
        if ($pk instanceof SetTitlePacket)
            return v361SetTitlePacket::fromLatest($pk);
        if ($pk instanceof SpawnParticleEffectPacket)
            return v361SpawnParticleEffectPacket::fromLatest($pk);
        if ($pk instanceof StartGamePacket)
            return v361StartGamePacket::fromLatest($pk, $this->blockMapping, $this->itemMapping);
        ...
        if ($pk instanceof UpdateAttributesPacket)
            return v361UpdateAttributesPacket::fromLatest($pk);
