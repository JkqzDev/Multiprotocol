...
class Main extends PluginBase {
    ...
    public function onEnable(): void {
        ...
        $regInterface = function (string $ip, int $port, bool $ipv6) use ($server, $translators, $net) {
            ...
            foreach ($translators as $translator) {
                $interface->registerTranslator($translator);
            }
    ...
    private function isLegacyPlayer(Player $player): bool {
        ...
        if (!$networkSession instanceof MVNetworkSession) {
            return false;
        }
        $pkTranslator = $networkSession->getPacketTranslator();

        if (($protocol = $pkTranslator::PROTOCOL_VERSION) === ProtocolInfo::CURRENT_PROTOCOL) {
            return false;
        }
        return true;
    }
